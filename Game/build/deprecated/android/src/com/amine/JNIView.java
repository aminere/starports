...
class JNIView extends GLSurfaceView
{
    ...
    private static class Renderer implements GLSurfaceView.Renderer
	{
        ...
        public void onDrawFrame(GL10 gl)
		{
			...
			{
				if(sActivity.m_HasFPSMode && !sActivity.m_PreviousHasFPSMode)
				{
					JNILib.onFPSModeAcquired();
					sActivity.m_PreviousHasFPSMode = true;
				}
				
				if(JNILib.isBuyFPSModeRequested())
				{
					Log.w(TAG, "isBuyFPSModeRequested..");
					sActivity.purchase("com.frantic.starports.extramode1");
					JNILib.resetBuyFPSModeRequest();
				}
				
				if(sActivity.m_Pinch)
				{
					long time = System.currentTimeMillis();
					double deltaTime = (time - sActivity.m_TouchTime)/1000.0;
					if(deltaTime < 0.05)
					{
						JNILib.zoom(sActivity.m_ZoomIn);
					}
					else
					{
						JNILib.zoomStop();
						sActivity.m_Pinch = false;
					}
				}
