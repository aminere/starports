...
{
	//! constructor
	OpenGLRenderTarget::OpenGLRenderTarget(const Vector2& vSize, int typeMask)
	...
	void OpenGLRenderTarget::GraphicLoad()
	{
		if(m_FrameBufferID)
		{
	...
	void OpenGLRenderTarget::GraphicUnload()
	{
	...
	//! resize the texture
	void OpenGLRenderTarget::Resize(const Vector2& vSize)
	{
		m_vSize = vSize;
		...
		if(m_DepthStencilRenderBufferID)
		{
			...
			GLenum format = (m_TypeMask & GraphicsDriver::BF_StencilBuffer) ? (GL_DEPTH24_STENCIL8) : (GL_DEPTH_COMPONENT16);
	...
	void OpenGLRenderTarget::BindTexture(int slot, int type)
	{
