...
#  ***** GPL LICENSE BLOCK *****
#
# Amine Rehioui added: WriteMeshVertexColors
#
...
def WriteMesh(Config, Object, Mesh):
    ...
    if Mesh.vertex_colors.active:
        if Config.Verbose:
            print("      Done\n      Writing Mesh Vertex Colors...")
        WriteMeshVertexColors(Config, Mesh)
    if Config.Verbose:
...
def WriteMeshVertexColors(Config, Mesh):
    Config.File.write("{}MeshVertexColors {{ // {} Vertex Colors\n".format("  " * Config.Whitespace, LegalName(Mesh.name)))
    Config.Whitespace += 1    

    VertexColors = Mesh.vertex_colors.active.data
    ...
    for Polygon in Mesh.polygons:
        Vertices = []
        for Vertex in [VertexColors[Vertex] for Vertex in Polygon.loop_indices]:
            Vertices.append(Vertex.color)
        if Config.CoordinateSystem == 1:
            Vertices = Vertices[::-1]
        for Vertex in Vertices:
            Config.File.write("{}{};{:9f};{:9f};{:9f};1.000000;;".format("  " * Config.Whitespace, Index, Vertex[0], Vertex[1], Vertex[2]))
            Index += 1
            if Index == VertexCount:
                Config.File.write(";\n")
            else:
    ...
    Config.Whitespace -= 1
    Config.File.write("{}}} //End of {} Vertex Colors\n".format("  " * Config.Whitespace, LegalName(Mesh.name)))
